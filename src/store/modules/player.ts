...
import type { ILyricInfo } from '@/utils/handleParseLyric.ts'
import type { PayloadAction } from '@reduxjs/toolkit'
import { getSongDetail, getSongLyric } from '@/api/player.ts'
import { parseLyric } from '@/utils/handleParseLyric.ts'
...
const playerSlice = createSlice({
  ...
  reducers: {
    ...
    changeLyricsAction(state, { payload }: PayloadAction<ILyricInfo[]>) {
...
>('currentSong', (id, { dispatch, getState }) => {
  ...
  getSongLyric(id).then((res: any) => {
    const lyricString = res.lrc.lyrics
...
>('playsong', (isNext, { dispatch, getState }) => {
  ...
  // 4.获取歌词数据
  getSongLyric(currentSong.id).then((res: any) => {
    const lyricString = res.lrc.lyrics
    const lyrics = parseLyric(lyricString)
    dispatch(changeLyricsAction(lyrics))
  })
